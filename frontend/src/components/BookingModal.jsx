...
export default function BookingModal({ onClose, discountApplied = false }) {
  ...
  // Click on backdrop (not the modal itself) closes it
  ...
  return (
    ...
    >
      <div
        className="relative bg-smoke border border-ash w-full max-w-2xl max-h-[90vh] flex flex-col"
      ...
      >
        {step === 5 ? (
          <>
            <button
              onClick={onClose}
              className="absolute top-3 right-3 w-9 h-9 flex items-center justify-center bg-ash hover:bg-copper text-bone/70 hover:text-bone rounded-full transition-colors z-20"
              aria-label="Close"
            >
            ...
            <div className="flex-1 overflow-y-auto flex flex-col items-center text-center px-6 py-6">
              ...
              <h3 className="font-heading text-xl md:text-2xl text-bone tracking-wider mb-1">
                You're Locked In.
              </h3>
              <p className="text-bone/50 text-[10px] font-heading uppercase tracking-[0.25em] mb-4">
                Booking Confirmed
              </p>

              <div className="border border-copper/40 bg-copper/5 px-4 py-2 mb-4 w-full max-w-sm">
                <p className="text-bone/50 text-[9px] font-heading uppercase tracking-[0.25em] mb-1">
                  Booking Reference
                </p>
              ...
              <div className="border border-ash w-full max-w-sm mb-4 text-left">
                <div className="divide-y divide-ash/60">
                  <div className="flex justify-between gap-4 px-4 py-2 text-xs">
                    <span className="text-bone/50 font-heading uppercase tracking-wider">Name</span>
                    <span className="text-bone text-right truncate">{form.name}</span>
                  </div>
                  <div className="flex justify-between gap-4 px-4 py-2 text-xs">
                    <span className="text-bone/50 font-heading uppercase tracking-wider">Service</span>
                    <span className="text-bone text-right">{selectedService?.name}</span>
                  </div>
                  <div className="flex justify-between gap-4 px-4 py-2 text-xs">
                    <span className="text-bone/50 font-heading uppercase tracking-wider">Date</span>
                    <span className="text-bone text-right">{formatDateDisplay(form.date)}</span>
                  </div>
                  <div className="flex justify-between gap-4 px-4 py-2 text-xs">
                    <span className="text-bone/50 font-heading uppercase tracking-wider">Time</span>
                    <span className="text-bone text-right">{form.time}</span>
                  </div>
                  <div className="flex justify-between gap-4 px-4 py-2 text-xs">
              ...
              <div className="w-full max-w-sm text-left mb-4">
                <p className="text-bone/50 text-[9px] font-heading uppercase tracking-[0.25em] mb-2 text-center">
                  What Happens Next
                </p>
                <ul className="space-y-1.5 text-[11px] text-bone/60">
                  <li className="flex gap-2">
                    <span className="text-copper">✦</span>
                    A confirmation has been sent to <span className="text-bone">{form.email}</span>
                  </li>
                  <li className="flex gap-2">
                    <span className="text-copper">✦</span>
                    You'll get a reminder 1 hour before your appointment
                  </li>
                  <li className="flex gap-2">
                    <span className="text-copper">✦</span>
                    Need to reschedule? Call us on <span className="text-bone">{shopInfo.phone}</span>
                  </li>
                </ul>
              </div>
            ...
            <div className="border-t border-ash p-3 text-center shrink-0">
              <button
                onClick={onClose}
                className="text-bone/50 hover:text-copper text-[10px] font-heading uppercase tracking-[0.25em] transition-colors"
              >
          ...
          <>
            ...
            <div className="flex border-b border-ash shrink-0">
              {["Service", "Master", "Date & Time", "Details"].map((label, i) => (
                <div
                  key={label}
                  className={`flex-1 text-center py-4 font-heading text-[10px] sm:text-xs uppercase tracking-widest ${
                    step === i + 1 ? "text-copper border-b-2 border-copper" : "text-bone/40"
                  }`}
                >
                  {label}
                </div>
            ...
            {discountApplied && (
              <div className="bg-copper/15 border-b border-copper/40 px-4 py-2 text-center shrink-0">
                <span className="text-copper font-heading text-[10px] uppercase tracking-widest">
                  ...
                  {services.map((s) => (
                    <button
                      ...
                      className={`w-full text-left p-4 border transition-all ${
                        form.service === s.id ? "border-copper bg-copper/10" : "border-ash hover:border-bone/30"
                      }`}
                    >
              ...
              {step === 2 && (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className={`w-full text-left p-4 border transition-all ${
                      form.barber === "any" ? "border-copper bg-copper/10" : "border-ash hover:border-bone/30"
                    }`}
                  >
                  ...
                  {barbers.map((b) => (
                    <button
                      ...
                      className={`w-full text-left p-4 border transition-all flex items-center gap-4 ${
                        form.barber === b.id ? "border-copper bg-copper/10" : "border-ash hover:border-bone/30"
                      }`}
                    >
              ...
              {step === 3 && (
                <div>
                  ...
                  <div className="grid grid-cols-7 gap-1 mb-1">
                    {WEEKDAYS.map((d) => (
                      <div key={d} className="text-center text-bone/40 text-[10px] font-heading uppercase py-2">
                        {d}
                      </div>
                    ))}
                  </div>
                    ...
                    {days.map((d) => {
                      ...
                      return (
                        <button
                          ...
                          className={`min-h-[44px] flex flex-col items-center justify-center text-sm font-heading border transition-all ${
                            closed
                              ? "text-bone/20 border-ash/30 cursor-not-allowed"
                              : selected
                              ? "bg-copper text-bone border-copper"
                              : "text-bone/80 border-ash hover:border-copper hover:bg-copper/10"
                          }`}
                        >
                          <span>{d.getDate()}</span>
                          {isToday(d) && (
                            <span className={`text-[7px] uppercase tracking-wider ${selected ? "text-bone" : "text-copper"}`}>
                              Today
                            </span>
                          )}
                        </button>
                  ...
                  <div className="flex gap-4 mt-4 text-[10px] font-heading uppercase tracking-widest text-bone/40">
                    <span className="flex items-center gap-2">
                      <span className="w-3 h-3 border border-copper inline-block" /> Selected
                    </span>
                    <span className="flex items-center gap-2">
                      <span className="w-3 h-3 border border-ash/30 inline-block" /> Closed
                    </span>
                  </div>
                  {form.date && (
                    <div className="mt-4 text-copper text-xs font-heading uppercase tracking-widest">
                      Selected: {formatDateDisplay(form.date)}
                    </div>
                  )}
                  {form.date && (
                    <div className="mt-6 pt-6 border-t border-ash">
                      <div className="text-bone/60 text-xs font-heading uppercase tracking-widest mb-3">
                        Available times
                      </div>
                      <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                        {times.map((t) => (
                          <button
                            ...
                            className={`py-2 text-xs font-heading tracking-wider border transition-all ${
                              form.time === t
                                ? "border-copper bg-copper text-bone"
                                : "border-ash text-bone/60 hover:border-bone/30 hover:bg-copper/10"
                            }`}
                          >
                            {t}
                          </button>
                  ...
                  {[
                    ...
                    <div key={input.field}>
                      ...
                      <input
                        ...
                        className="w-full bg-charcoal border border-ash text-bone p-3 focus:border-copper outline-none"
                      />
                    </div>
                  ))}
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-charcoal border border-ash text-bone p-3 focus:border-copper outline-none resize-none"
                    />
                  </div>
                  ...
                  <div className="border border-ash p-4 mt-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <p className="flex justify-between">
                        <span className="text-bone/60">Service</span>
                        <span className="text-bone">{selectedService?.name}</span>
                      </p>
                      <p className="flex justify-between">
                        <span className="text-bone/60">Duration</span>
                        <span className="text-bone">{selectedService?.duration} min</span>
                      </p>
                      <p className="flex justify-between">
                      ...
                      <p className="flex justify-between">
                        <span className="text-bone/60">Date</span>
                        <span className="text-bone">{formatDateDisplay(form.date)}</span>
                      </p>
                      <p className="flex justify-between">
                        <span className="text-bone/60">Time</span>
                        <span className="text-bone">{form.time}</span>
                      </p>
                    </div>
                    {discountApplied && (
            ...
            <div className="flex justify-between items-center gap-3 p-4 md:p-5 border-t border-ash shrink-0 bg-smoke">
              <div className="flex items-center gap-2">
                <button
                  onClick={goBack}
                  disabled={step === 1}
                  className={`font-heading uppercase tracking-widest text-xs px-3 py-3 transition-all ${
                    step === 1
                      ? "text-bone/20 cursor-not-allowed"
                      : "text-bone/60 hover:text-copper border border-ash hover:border-copper"
                  }`}
                >
                  &larr; Back
                </button>
                <button
                  onClick={onClose}
                  className="font-heading uppercase tracking-widest text-xs px-2 py-3 text-bone/40 hover:text-copper transition-colors"
                >
                  Cancel
                </button>
              </div>

              <div className="text-bone/40 text-[10px] font-heading uppercase tracking-widest hidden sm:block">
                Step {step} / 4
              </div>
              ...
              {step < 4 ? (
                <button
                  onClick={goNext}
                  disabled={!canProceed()}
                  className={`font-heading uppercase tracking-widest text-xs px-5 py-3 transition-all ${
                    canProceed()
                      ? "bg-copper text-bone hover:bg-gold hover:text-charcoal"
